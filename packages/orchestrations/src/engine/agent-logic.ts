...
export function processLLMResponse(
  ...
  if (reasoning?.trim()) {
    activities.push({ action: "reasoning", content: reasoning, simulated: reasoningSimulated || undefined, modelId, usage, intercepted });
    usageAttached = !!usage;
  }

  // Emit assistant text activity if there's content
  if (content.trim()) {
    activities.push({ action: "assistant_text", content, modelId, usage: !usageAttached ? usage : undefined, intercepted: !usageAttached ? intercepted : undefined });
