...
export default function Home() {
  // ── Shell (mounted in app/chat/layout.tsx, survives sibling navigation) ──
  ...
  // Approval state lives here (page-local) so the resolver Promise dies with
  // the conversation that triggered it. The wrapper is installed into the
  // shell's execute channel via installExecuteWrapper below.
  ...
  // Install the gated wrapper into the shell's execute channel. The channel
  // mounts once in the layout and reads the wrapper from a ref on every call,
  // so updating the wrapper here propagates without re-initializing the
  // channel.
  useEffect(() => {
  ...
  // The execute channel is mounted in the layout and sets this when a plugin
  // postMessage carries a workflowId. We read it here via shell to feed the
  // SSE stream + useOrchestrationConversation.
  ...
  const mcpInstances = shell.mcpInstances;
  ...
  // Sidebar state lives in the shell so the sidebar component itself can be
  // rendered by the layout. The header (still here in _home) just consumes the
  // shared toggle handler.
  const { sidebarOpen, setSidebarOpen, sidebarCollapsed, toggleSidebar } = shell;
  ...
  // TODO(Phase 4 follow-up): clean up dead state vars (figmaOAuth, enabledMcps, etc.)
    ...
    return (
    <>
        {/* The outer flex wrapper, the mobile backdrop, the sidebar, and the
            flex-1 column container are rendered by app/chat/layout.tsx. _home
            renders only header + body, which the layout slots inside its
            flex-1 column. */}
      ...
      {/* (Removed: orphan </div> from the inner flex-1 column wrapper now rendered by app/chat/layout.tsx) */}
