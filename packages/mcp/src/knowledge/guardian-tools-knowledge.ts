...
- **Effects (DROP_SHADOW, INNER_SHADOW)**: The \`effects\` array requires \`blendMode\` and \`visible\` fields: \`node.effects = [{ type: 'DROP_SHADOW', color: { r: 0, g: 0, b: 0, a: 0.25 }, offset: { x: 0, y: 2 }, radius: 4, spread: 0, visible: true, blendMode: 'NORMAL' }]\`. Without \`blendMode\` and \`visible\`, the validation will fail. Note: \`color\` in effects uses RGBA (with \`a\`), unlike fills which use RGB only.
- **Fill/stroke opacity**: For SOLID fills and strokes, do NOT put \`a\` (alpha) in the color object — it is not recognized. Use \`opacity\` on the paint instead: \`{ type: 'SOLID', color: { r: 0.5, g: 0.5, b: 0.5 }, opacity: 0.3 }\`. The color object only accepts \`{ r, g, b }\` in 0–1 range.
- **Corner radius**: Use \`node.cornerRadius = 8\` for uniform radius. For per-corner: \`node.topLeftRadius\`, \`node.topRightRadius\`, \`node.bottomLeftRadius\`, \`node.bottomRightRadius\`.
