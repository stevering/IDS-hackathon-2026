...
| `FIGMA_OAUTH_SCOPES` | Figma OAuth scopes override (optional) | `current_user:read,file_content:read,file_metadata:read` |
...
> **Note:** `FIGMA_CLIENT_ID` / `FIGMA_CLIENT_SECRET` are for standard Figma OAuth (REST API access). These tokens **cannot** access the remote MCP server at `mcp.figma.com/mcp`, which requires the `mcp:connect` scope — only available to whitelisted MCP clients (VS Code, Cursor, Claude Code, etc.) via Dynamic Client Registration (DCR). Third-party apps get 403 on the DCR endpoint. See the README troubleshooting section for details.
