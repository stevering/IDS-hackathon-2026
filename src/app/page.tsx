...
export default function Home() {
  ...
  // When loaded in OAuth popup after success/failure: notify opener and close
  const [oauthPopupStatus, setOauthPopupStatus] = useState<"success" | "failed" | null>(() => {
    if (typeof window === "undefined") return null;
    const p = new URLSearchParams(window.location.search);
    if (p.get("source") === "southleft-mcp" && (p.get("auth") === "success" || p.get("auth") === "failed"))
      return p.get("auth") as "success" | "failed";
    return null;
  });
  useEffect(() => {
    if (!oauthPopupStatus) return;
    if (window.opener) {
      try {
        window.opener.postMessage(
          { type: "southleft-mcp-auth", success: oauthPopupStatus === "success" },
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center p-6">
        <div className="text-center max-w-sm">
          {oauthPopupStatus === "success" ? (
            <>
              <p className="text-lg text-emerald-400 font-medium mb-2">Connexion réussie</p>
              <p className="text-sm text-white/60 mb-4">Figma Console est connecté. Vous pouvez fermer cette fenêtre.</p>
            </>
          ) : (
            <>
              <p className="text-lg text-amber-400 font-medium mb-2">Échec de la connexion</p>
              <p className="text-sm text-white/60 mb-4">Fermez cette fenêtre et réessayez depuis le plugin.</p>
            </>
          )}
          <button
            type="button"
  ...
  {southleftOAuth ? (
    ...
    <button
      onClick={async () => {
        ...
        const authUrl = "/api/auth/southleft-mcp";
        const inFigmaPlugin = typeof window !== "undefined" && window !== window.top;
        if (inFigmaPlugin) {
          ...
          if (popup) {
            ...
            let closedId: ReturnType<typeof setInterval>;
            const onMessage = (e: MessageEvent) => {
              if (e.data?.type === "southleft-mcp-auth") {
            ...
            pollId = setInterval(async () => {
              ...
              try {
                const r = await fetch("/api/auth/southleft-mcp/status", {
                  headers: tunnelSecret ? { "X-Auth-Token": tunnelSecret } : {},
                });
